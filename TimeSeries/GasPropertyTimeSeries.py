...
from ..PandasHelper.PandasHelper import DataModel
...
class GasPropertyTimeSeries(TimeSeries):
    ...
    def __getTimeSeries(self, rKpc: float):
        ...
        for timeMyr in self.timesMyr:
            ...
            value = self.pandasHelper.getDataFromCsv(self.basePath, self.gasProperty, rKpc, timeMyr)
            ...
            if (self.gasProperty == GasField.Luminosity):
                ...
                value = sp.quantities.total_quantity(self.field).d
            else:
                ds = yt.load('%s/perseus_merger_hdf5_plt_cnt_%04d'%(self.basePath, timeMyr))
                sp = ds.sphere((0,0,0), (rKpc, "kpc"))
                value = sp.quantities.weighted_average_quantity(self.field, self.weight).d
            del ds
            del sp

            # Write the calculated result into data base
            timeSeries.append(value)
            self.pandasHelper.writeDataIntoCsv(self.basePath, self.gasProperty, [DataModel(rKpc, timeMyr, value)])
