...
class TurbulenceHeatingVazzaTimeSeries:
    ...
    def getTimeSeries(
        ...
        '''
        if (powerMode == "total"):
            return self.__getHeatingTimeSeries(turbMode, simFile, calculationInfo)
        else:
            return self.__getHeatingPerVolumeTimeSeries(turbMode, simFile, calculationInfo)
        
    
    def __getHeatingPerVolumeTimeSeries(
        self,
        ...
        if (calculationInfo.shape == Shape.Sphere):
            volume = 4./3.*np.pi*u.Quantity(calculationInfo.rKpc, "kpc")**3
        else:
            volume = u.Quantity(2*calculationInfo.rKpc, "kpc")**3

        heatingTs = self.__getHeatingTimeSeries(mode, simFile, calculationInfo)
        heatingTs.yValue = (heatingTs.yValue/volume).to("erg/(s*cm**3)")
        return heatingTs
    
    
    def __getHeatingTimeSeries(
        self,
        mode: TurbulenceHeatingVazzaMode,
        simFile: SimFileModel,
        calculationInfo: TurbulenceHeatingVazzaTimeSeriesCalculationInfoModel
    ...
    def __calcHeatingRate(
        ...
        return PandasHelper().getDataOrUpdateDb(
            simPath=simFile.simPath,
            dbFieldName=dbFieldName,
            funcToGetValue=lambda: self.__geHeatingRateWithinRegion(
                shape=shape, 
